 ...
 *
 * Desktop: write button expands inline. Mobile: write mode shows a
 * detached input card above the (hidden) toolbar so the keyboard
...
export default function Toolbar({
  ...
  useEffect(() => {
    if (isWriteMode) {
      ...
      // Mobile input (slight delay for render)
  ...
  return (
    <>
      {/* ────────────────────────────────────────────────────────── */}
      {/*  MOBILE WRITE OVERLAY — input detached from toolbar       */}
      {/*  Only visible on screens < sm (640px) when write is active */}
      {/* ────────────────────────────────────────────────────────── */}
      {isWriteMode && (
        <div className="fixed inset-x-0 bottom-0 z-50 p-4 sm:hidden">
          <div className="bg-neutral-900 rounded-xl border border-amber-800/40 shadow-2xl p-4 space-y-3">
            {/* Input row */}
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="bg-transparent text-white text-base outline-none flex-1 min-w-0 placeholder:text-neutral-500"
              />
            </div>
            {/* Actions row */}
            <div className="flex items-center justify-between">
              <button
                ...
                className="px-3 py-1.5 text-neutral-500 text-xs hover:text-white transition-colors"
              >
                Cancel
              </button>
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-4 py-1.5 bg-amber-700 hover:bg-amber-600 disabled:bg-neutral-700 disabled:text-neutral-500 text-white text-sm font-medium rounded transition-colors whitespace-nowrap"
                >
      ...
      <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center gap-3 ${
        ...
        <div className="flex items-center gap-1 px-3 py-2 bg-neutral-900 rounded-xl border border-neutral-800 shadow-2xl">
          {/* Write — expands into input when active (desktop only) */}
          <div className="relative">
            ...
            {isWriteMode ? (
              /* Expanded state — DESKTOP ONLY (mobile uses overlay above) */
              <div className="hidden sm:flex items-center gap-2 h-10 px-3 rounded-lg bg-amber-900/30 border border-amber-800/40">
                ...
                <input
                  ...
                  onKeyDown={handleKeyDown}
                  placeholder="Type a word or sentence..."
                  className="bg-transparent text-white text-sm outline-none w-64 placeholder:text-neutral-500"
                />
                {wordCount > 0 && (
